...
class AW_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action {
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
        	...
        	if(isset($_FILES['post_image']['name']) && $_FILES['post_image']['name'] != '') {	// strip code added v1.2.3 to add field image post for post form
        		try {
        			/* Starting upload */
        			...
        			$path = Mage::getBaseDir('media') . DS."aw_blog";
        		...
        		$data['post_image'] = 'aw_blog/'.$_FILES['post_image']['name'];
        	}
        	else
        	{
        	...
        	}																					//end v1.2.3 to add field image post for post form
